...
import 'package:sqflite/sqflite.dart' ; //used to access sql lite database
...
class DbHelper{
  ...
  static final DbHelper _dbHelper = DbHelper._internal(); //declare _dbHelper instance as final, means that is a single-assignment variable. Once assigned its value can not change
  DbHelper._internal();
  ...
  Future<List> getDoc(int id) async{
    Database db = await this.db;
    var r = await db.rawQuery("SELECT * FROM $tblDocs WHERE $docId="+id.toString()+" " );
    return r;
  }

  //Gets a String based on a string payload
  Future<List> getDocFromStr(String payload) async {
    List<String> p=payload.split("|"); //the method receives a String object payload, which contains two important bits od data
    if(p.length==2){
      Database db=await this.db;
  ...
  Future<int> getDocsCount() async{
    Database db = await this.db;
    var r = Sqlflite.firstIntValue(  
      await db.rawQuery("SELECT COUNT(*) FROM $tblDocs")
    );
    return r;
  }

  //Get the max document id availble on the database
